...
install_deps() {
  ...
  # Modern CLI tools
  command -v zoxide &>/dev/null || pkgs+=(zoxide)
  command -v eza    &>/dev/null || pkgs+=(eza)
  command -v micro  &>/dev/null || pkgs+=(micro)
  command -v fzf    &>/dev/null || pkgs+=(fzf)
  command -v jq     &>/dev/null || pkgs+=(jq)

  # bat: package 'bat', binary differs (batcat on Debian, bat on Arch)
  { command -v batcat &>/dev/null || command -v bat &>/dev/null; } || pkgs+=(bat)

  # fd: package name differs (fd-find on Debian → fdfind binary; fd on Arch)
  if [[ $pm == apt ]]; then
    { command -v fdfind &>/dev/null || command -v fd &>/dev/null; } || pkgs+=(fd-find)
  else
